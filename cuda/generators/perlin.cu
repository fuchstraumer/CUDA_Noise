...
__device__ __constant__ float g_randomVectors[256 * 4] ={
...
__device__ float lerp(const float n0, const float n1, const float a){
	return ((1.0f - a) * n0) + (a * n1);
}

__device__ float sCurve3(const float a){
...
__device__ float grad3d(const float fx, const float fy, const float fz, const int ix, const int iy, const int iz, const int seed) {
...
__device__ float perlin3d(const float3 p, const int seed, noise_quality qual) {
	/*
		...
		modified to work best on the GPU. Primarily, an emphasis has been given to 
		the avoidance of using any local variables unless absolutely necessary.

		Because of this, this code is practically obsfucated. This was not the intent:
		merely a necessity of the issues inherent with register overspill and memory
		shortages for functions local to a single thread (like this one).

		Please see LibNoise's source, "NoiseGen.cpp", if you'd like to see code that's
		actually readable. FastNoise works as well: its heavily derived from LibNoise
		as well.
	...
	*/
	...
	/*
		...
		1. Floor input position, use it to find relative position in gradient-vector space. (L333
		2. Get noise values at each vertex of cube. (
